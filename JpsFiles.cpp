...
void JpsFile::readFile()
{
	...
	for (int i = 0; i < fileLen; i++){
		...
		if (i <= strLen){
			...
			if (test == "~~") {JpsData* jps = new ReceiverTime("RT", strLen, str);delete jps;}
			if (test == "::") {	LOG("Have found ET");}
			if (test == "RD") {JpsData* jps = new ReceiverDate("RD", strLen, str);delete jps;}
			if (test == "TO") {LOG("Have found TO");}
